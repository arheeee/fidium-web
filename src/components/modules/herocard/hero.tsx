...
const Hero: React.FC<HeroProps> = ({ block }) => {
  ...
  const imageUrl = block.heroImage.length ? getImageUrl(block.heroImage[0].url ?? '') : '';
  const mobileImageUrl = block.heroMobileImage.length ? getImageUrl(block.heroMobileImage[0].url ?? '') : '';

  return (
    <section className="hero__section">
      <div className="hero__container">
        <div className={`hero__background ${mobileImageUrl ? 'hero__has-mobile-bg' : ''}`}>
          {imageUrl && (
            <Image
              src={imageUrl}
          ...
          {mobileImageUrl && (
            <Image
              src={mobileImageUrl}
              alt={block.heroMobileImage[0].alt || block.heroMobileImage[0].title}
        ...
        <div className="hero__content">
          <div className="hero__copy-wrap">
            ...
            {block.heroSubHeader && <p className="hero__subtitle">{block.heroSubHeader}</p>} 
          </div>
